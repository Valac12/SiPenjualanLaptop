...
@section('content')
 ...
 @if(session()->has('successCreateKasir'))
 <div class="container-fluid py-2">
    <div class="alert alert-success alert-dismissible fade show" role="alert">
    {{ session('successCreateKasir') }}
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
  </div>
  @endif
  {{-- alert berhasil dihapus --}}
  @if(session()->has('successDelKasir'))
  <div class="container-fluid py-2">
    <div class="alert alert-warning alert-dismissible fade show" role="alert">
    {{ session('successDelKasir') }}
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
  </div>
  @endif
 ...
 <div class="container-fluid w-100 ">
  <div class="d-flex justify-content-end py-2">
    <button class="btn btn-primary btn-md border-3" type="button" title="Tambah Data" data-bs-toggle="modal" data-bs-target="#modalKasir">+Tambah Data</button>
  </div>
        ...
        <tbody>
          @foreach($userKasir as $uk)
            <tr>
                ...
                <td>{{ $uk->level_name }}</td>
                <td>
                  <a class="badge bg-warning border-0 m-1" href="/kelolaKasir/{{ $uk->id }}">
                      <svg class="svg-icon svg-icon-sm svg-icon-heavy">
                          <use xlink:href="#eye-1"> </use>
                      </svg>
                  </a>
                    <form action="/kelolaKasir/{{ $uk->id }}" method="post" class="d-inline">
                      @method('delete')
                      @csrf
...
<!-- Modal Create -->
<div class="modal fade" id="modalKasir" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="/kelolaKasir" method="post" class="was-validated needs-validation" novalidate>
            @csrf
